...
//
// $Id: SphinxClientProvider.phpider.php 13555 2015-06-24 10:28:13Z fengxu $
//
...
class SphinxClientProvider extends Provider
{
     ...
     * 'filterVal' => 1
     * )
     * ), // sphinx SetSelect 主要用于组合条件表达式的过滤,如:OR
	 * 'conds' => array(
	 * 'eq' => array('sign' => 1),
	...
	public function search(array $rule)
	{
	    ...
	    $this->ResetFilters();
        $this->_parseLimit($rule);
        $this->_parseOrderBy($rule);
	    $query = $this->_parseConds($rule);
        if (isset($rule['select']) && is_array($rule['select'])) {
            $exps = array();
            foreach ($rule['select'] as $alias => $define) {
                $filterVal = isset($define['filterVal']) ? (int)$define['filterVal'] : 1;
                if (is_array($define) && isset($define['exp'])) {
                    $this->SetFilter($alias, array($filterVal));
                    $exps[] = "({$define['exp']}) as $alias";
                }
            }
            if ($exps) {
                $this->SetSelect(join(', ', $exps));
            }
        }
	    ...
	    if ($result && $result['total']) {
	        ...
	        $res['error'] = $this->GetLastError();
	    }
	    return $res;
	}

	/**
	 * 解析查询条件中的like定义，支持完全、前置、后置匹配
	 * 
	 * @todo 考虑先调用BuildKeywords分词然后在查询的必要性
	 * 
	 * @param array $defineArray
	 * @return string
	 */
	protected function _parseLike(array $defineArray)
	{
	    $likeConds = array();
	    foreach ($defineArray as $field => $define) {
	        ...
	        $kw = $this->EscapeString($kw);
            $prefix = ($mode == 1 || $mode == 3) ? '^' : '';
            $suffix = ($mode == 2 || $mode == 3) ? '$' : '';
            $likeConds[] = "@$field ($prefix$kw$suffix)";
	    }
	    return join('|', $likeConds);
	}
	 ...
	 * @return void
	 */
    protected function _parseOrderBy(array $rule)
    {
        $orderByConds = array();
        if (isset($rule['order']) && is_array($rule['order'])) {
            foreach ($rule['order'] as $order => $fields) {
                foreach ($fields as $field) {
                    $orderByConds[] = $field == 'primaryKey' ? "@id $order" : "$field $order";
                }
            }
        }
        if ($orderByConds) {
            $this->SetSortMode(SPH_SORT_EXTENDED, join(', ', $orderByConds));
        }
    }

    protected function _parseLimit(array $rule)
    {
        $offset = 0;
        $limit = 1;
     ...
     * @return string
     */
	protected function _parseConds(array $rule)
	{
        $likeQuery = '';
	    if (isset($rule['conds']) && is_array($rule['conds'])) {
	        foreach ($rule['conds'] as $operator => $fields) {
	            switch ($operator) {
	                case 'eq':
	                case 'neq':
	                case 'in':
	                case 'nin':
	                    foreach ($fields as $field => $values) {
	                        $values = (array)$values;
	                        $exclude = in_array($operator, array('neq', 'nin')) ? true : false;
	                        $this->SetFilter($field, $values, $exclude);
	                    }
	                    break;
	                case 'scope':
	                    foreach ($fields as $field => $scope) {
	                        if (is_float($scope['min']) || is_float($scope['max'])) {
	                            $this->SetFilterFloatRange($field, $scope['min'], $scope['max']);
	                        } else {
	                           $this->SetFilterRange($field, $scope['min'], $scope['max']);
	                        }
	                    }
	                    break;
	                case 'like':
	                    $likeQuery = $this->_parseLike($fields);
	                    break;
	                    default:
	            }
	        }
